/**
 * Motor CONVERSACIONAL del asistente interno de Alpha Gestion ("Alfi") —
 * copiado del motor de StockFlow (Flowy, v0.1.86) con la base de conocimiento
 * propia de Alpha. 100% offline, sin IA externa, sin clave, sin costo. Vive en
 * el SERVIDOR Fastify (a diferencia de StockFlow que lo corre en el main):
 * asi tambien puede responder en el monitor de elaboracion o la PWA.
 *
 ...
 */
import { datosIntents as intentsData, datosManual as manualData } from './conocimiento';
...
const BASE_SYNONYMS: Synonym[] = [
  ...
  { term: 'recibo', aliases: ['comprobante', 'ticket', 'factura', 'remito'] },
  { term: 'venta', aliases: ['vender', 'cobrar', 'facturar', 'cobro'] },
  ...
  { term: 'precio', aliases: ['valor', 'costo', 'lista'] },
  { term: 'pedido', aliases: ['encargo', 'nota de pedido', 'talonario'] },
  { term: 'elaborar', aliases: ['elaboracion', 'producir', 'produccion', 'fabricar', 'hacer', 'tanda'] },
  { term: 'orden', aliases: ['ordenes', 'orden de produccion'] },
  { term: 'lote', aliases: ['trazabilidad', 'partida'] },
  { term: 'insumo', aliases: ['insumos', 'materia prima', 'harina', 'dulce de leche'] },
  { term: 'docena', aliases: ['docenas', 'caja', 'cajas', 'bolsa', 'presentacion'] },
  { term: 'cheque', aliases: ['cheques', 'cartera'] },
  { term: 'vendedor', aliases: ['revendedor', 'viajante'] },
  { term: 'whatsapp', aliases: ['wsp', 'wpp', 'guasap', 'mensaje'] },
...
function buildIndex(): Index {
  ...
  const topSuggestions = realIntents.length
    ...
    : ['Como cargo un pedido?', 'Como elaboro un pedido?', 'Como cobro una venta?'];
...
export function answerQuestion(question: string, convId = 'default'): AssistantAnswer {
  ...
  // "sí" y el asistente volvia a saludar). Si hay contexto pendiente y el mensaje es
  ...
  if ((content.length === 0 || (raw.length <= 2 && has(raw, GREETING))) && !(pendingCtx && affirmish)) {
    ...
    return metaReply(idx, 'saludo', '¡Hola! Soy Alfi, tu asistente de Alpha Gestion. Escribime en qué te puedo ayudar.');
  }
  ...
  if (phraseHas(low, BYE_PH)) return metaReply(idx, 'despedida', '¡Listo! Cualquier cosa me abrís de nuevo. Exitos con la fabrica 💪');
  if (phraseHas(low, WHOAREYOU_PH)) return metaReply(idx, 'quien-sos', 'Soy Alfi, el asistente de Alpha Gestion 🤖 Te enseño a usar cada parte del sistema, paso a paso.');
  if (phraseHas(low, WHATCANDO_PH)) return metaReply(idx, 'que-podes-hacer', 'Te ayudo con todo el sistema: pedidos, elaboracion, stock, ventas, caja, cheques, clientes y mas. Preguntame lo que quieras.');
  if (phraseHas(low, HUMAN_PH)) return metaReply(idx, 'pedir-humano', 'Puedo ayudarte con casi todo del uso del sistema. Si es algo que no logro resolver, anotá la duda y consultala con BPSG Sistemas, que te instalo el sistema.');
  ...
  if (phraseHas(low, OFFTOPIC_PH)) return metaReply(idx, 'charla-fuera-de-tema', '¡Jaja! De eso no sé, yo soy para ayudarte con Alpha Gestion 😊 ¿Con qué parte del sistema te doy una mano?');
